...
namespace nimbus{
                    ...
                    int reconnectIntents = 3, double imgBufSize =10) 
    {
        ...
        for(size_t i = 0; i < 286*352; i++) this->_uX.push_back(unitX[i] / spread);
        for(size_t i = 0; i < 286*352; i++) this->_uY.push_back(unitY[i] / spread);
        for(size_t i = 0; i < 286*352; i++) this->_uZ.push_back(unitZ[i] / spread);
    ...
    std::vector<std::vector<float>> WebSocketClient::getImage(){
        ...
        if((this->_connected != false) && (_queueFront != ""))
        {
            ...
            if (this->imgType != 0)
            {
                ...
                for(size_t i =0; i < 286*352; i++) radial.push_back((float)imgDecoded.radial[i]/65535*this->_UR);
    ...
    ImageDecoded WebSocketClient::create(std::string buffer)
    {
        ...
        this->imgType = (int)headers[HeaderImgType];
        ...
        if(imgType == NimbusImageRaw)
        {
            ...
            */
            ...
            if(imgType & NimbusImageConf)
            {
                ...
                for(int i = 0; i<286*352; i++)
                {
                    //std::cout <<"Confidence at: " << i << " is: " << (int)this->confid[i] << std::endl;
